...
function myfbconnect_install()
{
    ...
    $columnsToCheck = ['fbavatar', 'fbsex', 'fbdetails', 'fbbday', 'fblocation', 'VARCHAR(32) NOT NULL DEFAULT 0' => 'myfb_uid'];
    $columnsToAdd = '';

    // Check if columns are already there (this prevents duplicate installation errors)
    foreach ($columnsToCheck as $type => $name) {
        ...
        if (!$db->field_exists($name, 'users')) {
            ...
            $columnsToAdd .= "`{$name}` $type,";

        }

    }

    $columnsToAdd = rtrim($columnsToAdd, ',');

    // Insert our Facebook columns into the database
    if ($columnsToAdd) {
        $db->query("ALTER TABLE " . TABLE_PREFIX . "users ADD ({$columnsToAdd})");
    }
